...
#include "md5.h"
#include "md5.c"
int wrapfs_encode_block_test(const char *from,char *to,int len);
int wrapfs_decode_block_test(const char *from,char *to,int len);
...
static unsigned long pbx[18];

static unsigned long sbx[4][256];
...
void blowfishKeyInit(int uid,char *filename)
{
	char key[100];
	int i=0,j=0;
			   ...
			   size_t count, loff_t *ppos)
{
	...
	char *begin=NULL,*decoded_buf = NULL;
	char *decode_cp = buf;
	int i;
	unsigned int page,size;
	size_t decode_count = 0;

	decoded_buf = (char*)kmalloc(count,GFP_KERNEL);

	if (IS_ERR(decoded_buf))
	{
		err = PTR_ERR(decoded_buf);
		decoded_buf=NULL;
		return err;
	}
	begin=decoded_buf;
	...
	blowfishKeyInit(file->f_owner.uid.val,dentry->d_iname);
//	getkey("hello");
	...
	if (err >= 0)
	{
		...
		for (i=0;i<page;i++)
		{
			//err = wrapfs_decode_block_test(decode_cp,decoded_buf,pAGE_SIZE);
			blowfish_decrypt(decoded_buf,decode_cp);
			err=8;
			if (err<0) return err;
			decode_count+=pAGE_SIZE;
			decoded_buf+=pAGE_SIZE;
			decode_cp+=pAGE_SIZE;
		}

		if (size)
		{
			//err = wrapfs_decode_block_test(decode_cp,decoded_buf,size);
			blowfish_decrypt(decoded_buf,decode_cp);
		//	memcpy(decoded_buf,decode_cp,size);
		//	if (err<0)  return err;
			decode_count+=pAGE_SIZE;
		}

//		if (*(begin+decode_count)==0) decode_count--;

	}
	memcpy(buf,begin,decode_count);
...
int wrapfs_decode_block_test(const char *from,char *to,int len)
{
	...
	unsigned char *incp = (unsigned char *) from;
	unsigned char *outcp = (unsigned char *) to;
	int i;
	char c;
	for (i=0;i<len;i++)
	{
...
/*
int wrapfs_decode_block(const char *from,char *to,int len)
{
	int n=0;
	unsigned char iv[8];
	...
	BF_set_key(&bf_key,16,key_array);
...
*/
/*
int wrapfs_encode_block(struct file *file,char __user *buf,size_t count,loff_t *ppos)
{
	int err;
	int i;
	struct file *lower_file;
	unsigned int lo_pos = *ppos;

	unsigned int begin_chip_pos = lo_pos%BLOCK_SIZE;
	unsigned long begin_block_no = lo_pos/BLOCK_SIZE;

	unsigned int begin_chip_size = BLOCK_SIZE-begin_chip_pos;

	loff_t being_block_pos = lo_pos - begin_chhip_pos;

	unsigned int end_chip_pos = (lo_pos+count)%BLOCK_SIZE;
	unsigned int block_no = (count-begin_chip_size)/BLOCK_SIZE;
	loff_t end_block_pos = lo_pos + count -end_chip_pos;

	loff_t mid_block_pos = lo_pos + begin_chip_size;

	char *chip_buf = NULL;
	char *context_buf = NULL;
	char *end_chip_buf = NULL;

	lower_file = wrapfs_lower_file(file);
	err = vfs_read(lower_file,chip_buf,BLOCK_SIZE,&begin_block_pos);
	if (err<0) goto out;

	err = wrapfs_decode_block(chip_buf,tmp_buf,BLOCK_SIZE);
}
*/
			    ...
			    size_t count, loff_t *ppos)
{
	...
	unsigned int page = count / pAGE_SIZE;
	unsigned int size = count % pAGE_SIZE;

	lower_file = wrapfs_lower_file(file);

	/*write test*/
    encoded_buf = (char*)kmalloc(count,GFP_KERNEL);
	tmp_buf = (char*)kmalloc(pAGE_SIZE,GFP_KERNEL);

	if (IS_ERR(encoded_buf))
	{
		err = PTR_ERR(encoded_buf);
		encoded_buf = NULL;
		return err;
	}
	...
	begin = encoded_buf;
    //err = wrapfs_encode_block_test(buf, encoded_buf, count);

	for (i=0;i<page;i++)
	{
		//err = wrapfs_encode_block_test(encode_cp,encoded_buf,pAGE_SIZE);
		blowfish_encrypt(encode_cp,encoded_buf);
		if (err<0) return err;
		encode_count+=pAGE_SIZE;
		encode_cp+=pAGE_SIZE;
		encoded_buf+=pAGE_SIZE;
	}

	if (size) 
	{
		//err = wrapfs_encode_block_test(encode_cp,encoded_buf,size);
		memset(tmp_buf,0,pAGE_SIZE);
		memcpy(tmp_buf,encode_cp,size);
//		memcpy(encoded_buf,tmp_buf,size);
		blowfish_encrypt(tmp_buf,encoded_buf);
		if (err<0) return err;
		encode_count += pAGE_SIZE;
	}

	memcpy(buf,begin,encode_count);
//	encoded_buf = buf;
	err = vfs_write(lower_file, buf, encode_count, ppos);
//	encoded_buf = kmalloc(count,GFP_ATOMIC);
//	encoded_buf = (char*)kmalloc(count,GFP_KERNEL);

//	unsigned char *incp = (unsigned char *) buf;
//	unsigned char *outcp = (unsigned char *) encoded_buf;

/*	for (i=0;i<count;i++) 
	{		if (*(buf+i)<='z' && *(buf+i)>='a')
			*(encoded_buf+i)='h';
		else *(encoded_buf+i)=*(buf+i);
		*outcp =*incp;
		outcp++;
		incp++;
	}
*/	
//	encoded_buf = buf;
//	err = vfs_write(lower_file, encoded_buf, count, ppos);
